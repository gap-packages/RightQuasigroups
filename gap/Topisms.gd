...
DeclareOperation( "RQ_IsAlgebraHomotopism", [ IsObject, IsMapping, IsMapping, IsMapping ] );
...
DeclareOperation( "RQ_IsAlgebraIsotopism", [ IsObject, IsMapping, IsMapping, IsMapping ] );
...
# RQ_IsAlgebraHomotopism( category, f, g, h )
DeclareOperation( "RQ_IsAlgebraAutotopism", [ IsObject, IsMapping, IsMapping, IsMapping ] );
...
DeclareOperation( "RQ_AlgebraTwist", [ IsObject, IsList ] );
...
DeclareOperation( "RQ_AlgebraIsotope", [ IsObject, IsList ] );
...
DeclareOperation( "RQ_IsotopismAlgebras", [ IsObject, IsRightQuasigroup, IsRightQuasigroup, IsBool ] );
...
DeclareOperation( "RQ_AlgebrasUpToIsotopism", [ IsObject, IsList, IsBool ] );
...
#! @Description As &GAP; objects, autotopisms of right quasigroups are multiplicative elements with inverse, represented as positional objects.
...
#! @Arguments Q,f,g,h
#! @Returns the autotopism &GAP; object. This is a non-checking version, only for experts. 
DeclareGlobalFunction( "AutotopismObject@" );
#! @Arguments atop
#! @Returns the ambient right quasigroup of an autotopism. Notice that autotopisms are no mappings on their ambient right quasigroup.
...
#! @Arguments Q,a,b
#! @Returns an isotopism from $Q$ to the principal loop isotope $Q_{a,b}$, if such an isomorphism exists. If not, then `fail` is returned.
DeclareOperation( "AutotopismFromPrincipalLoopIsotope", [ IsLoop, IsLoopElement, IsLoopElement ] );

#! @BeginGroup
#! @GroupTitle AtopOn3nElms@, AtopOnnSquareElms@

#! Let $Q$ be a right quasigroup and $\tau =(f_1,f_2,f_3)$ an autotopism of $Q$. There are four sets where $\tau$ has a natural faithful action. 
#! <List>
#! <Item>$\Omega_1=Q\dot\cup Q\dot\cup Q$ of the disjoint union of three copies of $Q$. Formally, the elements of $\Omega_1$ 
#! are pairs $(x,i)$, $x\in Q$, $i \in \{1,2,3\}$. The image of $(x,i)$ is $(x^{f_i},i)$. If $|Q|=n$ and $x$ is the $j$th 
#! element of $Q$, then $(x,i)$ can be identified with $j+(i-1)n \in \{1,\ldots,3n\}$. In this way, $\tau$ acts on 
#! $\{1,\ldots,3n\}$. This action is implemented in <C>AtopOn3nElms&#64;</C>. </Item>
#! <Item>$\Omega_2=Q\times Q$ or $\Omega_3=Q\times Q\times Q$. For $x_1,x_2,x_3 \in Q$, define $(x_1,x_2)^\tau=(x_1^{f_1},x_2^{f_2})$ 
#! and $(x_1,x_2,x_3)^\tau=(x_1^{f_1},x_2^{f_2},x_3^{f_3})$. If $|Q|=n$ and $x_i\in \{1,\ldots,n\}$, then the action is given by 
#! identifying $Q$ with $\{1,\ldots,n\}$. This action is implemented in <C>AtopOnnSquareElms&#64;</C>. </Item>
#! <Item>If $\tau$ is a proper autotopism of $Q$, then it acts on the subset $\Omega_3' = \{(x,y,x\cdot y) : x,y \in Q\}$ of $\Omega_3$.</Item>
#! </List>
...
#! @Arguments p,atop
DeclareOperation( "AtopOnnSquareElms@", [ IsList, IsRightQuasigroupAutotopismObject ] );
#! @EndGroup

#! @BeginGroup
#! @GroupTitle AutotopismGroupByGenerators
#! @Arguments Q,gens
DeclareOperation( "AutotopismGroupByGenerators", [ IsRightQuasigroup, IsList  ] );
#! @Arguments gens
#! @Returns the group of autotopisms of <A>Q</A>, generated by <A>gens</A>. If <A>Q</A> is not given, then <A>gens</A> may not be empty.
...
#! @Arguments Q
#! @Returns the group of autotopisms of the loop <A>Q</A>. 
...
#! @BeginGroup
#! @GroupTitle Isotope invariants of loops

#! <P/>Let $(Q,\cdot)$ be a quasigroup with elements $x_1,\ldots,x_n$. For $i\in \{1,\ldots,n\}$, let $\lambda_i$ 
#! denote the multiset of the cycle structures of the permutations <C>RightTranslation( x_i )^-1 * RightTranslation( x_j )</C>, 
#! $j\in  \{1,\ldots,n\}$. Similarly, $B_i$ is  the multiset of the cycle structures of the permutations 
#! <C>LeftTranslation( x_i )^-1 * LeftTranslation( x_j )</C>, $j\in  \{1,\ldots,n\}$. The $n$-tuples $
#! A=(A_1,\ldots,A_n)$ and $B=(B_1,\ldots,B_n)$ are the <E>right and left isotope invariants</E> of $Q$. 
#! The pair $(A,B)$ is the <E>isotope invariant</E> of $Q$.
...
#! @Arguments Q,S,a,b
#! @Returns Let $a,b$ be the $i$th and $j$th elements of $S$. Let $A=(A_1,\ldots)$, $B=(B_1,\ldots)$ be 
#! rigth and left isotope invariants of $Q$. Similarly, let $C=(C_1,\ldots)$, $D=(D_1,\ldots)$ be rigth 
#! and left isotope invariants of $S$. Returns true if $A_1=C_i$ and $B_1=D_j$. 
#! @Description Notice that this is a necessary condition for $Q$ be isomorphic to the principal isotope $S_{a,b}$. 
